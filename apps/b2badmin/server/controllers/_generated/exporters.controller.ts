...
export const exportersController = new Elysia({ prefix: "/exporters" })
  ...
  .get(
    "/",
    ({ query, auth, db }) => exportersService.findAll(query, { db, auth }),
    { query: ExportersContract.ListQuery }
  )
  .post(
    "/",
    ({ body, auth, db }) => exportersService.create(body, { db, auth }),
    { body: ExportersContract.Create }
  )
  .delete(
    "/:id",
    ({ params, auth, db }) => exportersService.delete(params.id, { db, auth }),
