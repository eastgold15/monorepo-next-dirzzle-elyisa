...
export const inquiryitemsController = new Elysia({ prefix: "/inquiryitems" })
  ...
  .get(
    "/",
    ({ query, auth, db }) => inquiryItemsService.findAll(query, { db, auth }),
    { query: InquiryItemsContract.ListQuery }
  )
  .post(
    "/",
    ({ body, auth, db }) => inquiryItemsService.create(body, { db, auth }),
    { body: InquiryItemsContract.Create }
  )
  .delete(
    "/:id",
    ({ params, auth, db }) =>
      inquiryItemsService.delete(params.id, { db, auth }),
