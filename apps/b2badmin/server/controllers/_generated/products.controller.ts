...
export const productsController = new Elysia({ prefix: "/products" })
  ...
  .get(
    "/",
    ({ query, auth, db }) => productsService.findAll(query, { db, auth }),
    { query: ProductsContract.ListQuery }
  )
  .post(
    "/",
    ({ body, auth, db }) => productsService.create(body, { db, auth }),
    { body: ProductsContract.Create }
  )
  .delete(
    "/:id",
    ({ params, auth, db }) => productsService.delete(params.id, { db, auth }),
