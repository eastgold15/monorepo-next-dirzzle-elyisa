...
export const quotationitemsController = new Elysia({
  ...
  .get(
    "/",
    ({ query, auth, db }) => quotationItemsService.findAll(query, { db, auth }),
    { query: QuotationItemsContract.ListQuery }
  )
  .post(
    "/",
    ({ body, auth, db }) => quotationItemsService.create(body, { db, auth }),
    { body: QuotationItemsContract.Create }
  )
  .delete(
    "/:id",
    ({ params, auth, db }) =>
      quotationItemsService.delete(params.id, { db, auth }),
