...
export const roleController = new Elysia({ prefix: "/role" })
  .use(dbPlugin)
  .use(authGuardMid)
  .get("/", ({ query, auth, db }) => roleService.findAll(query, { db, auth }), {
    query: RoleContract.ListQuery,
  })
  .post("/", ({ body, auth, db }) => roleService.create(body, { db, auth }), {
    body: RoleContract.Create,
  })
  .delete(
    "/:id",
    ({ params, auth, db }) => roleService.delete(params.id, { db, auth }),
