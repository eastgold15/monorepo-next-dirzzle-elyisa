...
export const verificationController = new Elysia({ prefix: "/verification" })
  ...
  .get(
    "/",
    ({ query, auth, db }) => verificationService.findAll(query, { db, auth }),
    { query: VerificationContract.ListQuery }
  )
  .post(
    "/",
    ({ body, auth, db }) => verificationService.create(body, { db, auth }),
    { body: VerificationContract.Create }
  )
  .delete(
    "/:id",
    ({ params, auth, db }) =>
      verificationService.delete(params.id, { db, auth }),
