        ...
        return await ctx.db.transaction(async (tx) => {
            ...
            await tx
                ...
                .set({
                    ...
                    masterCategoryId,
                    // 将 "root" 或空值转为 null
                    siteCategoryId: (siteCategoryId && siteCategoryId !== "root") ? siteCategoryId : null,
                })
                .where(eq(attributeTemplateTable.id, templateId));
            ...
            if (fields && fields.length > 0) {
                for (const field of fields) {
                    const { inputType, isRequired, options, value, code, key, isSkuSpec } = field;
                    ...
                    } else if ((inputType === "text" || inputType === "number") && value) {
