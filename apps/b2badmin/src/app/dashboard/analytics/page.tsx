...
const generateMockData = (role: string | null) => {
  const baseData = {
    totalProducts: role === 'salesperson' ? 12 : role === 'factory_admin' ? 156 : 1247,
    totalOrders: role === 'salesperson' ? 28 : role === 'factory_admin' ? 892 : 5634,
    totalRevenue: role === 'salesperson' ? 12840 : role === 'factory_admin' ? 456780 : 3456789,
    totalUsers: role === 'salesperson' ? 1 : role === 'factory_admin' ? 45 : 234,
  };
        ...
        <SidebarInset>
          ...
          <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
            <div className="space-y-6">
              {/* Header */}
              <div>
                <h1 className="font-bold text-2xl text-slate-900">Analytics Dashboard</h1>
                <p className="mt-1 text-slate-500">
                  ...
                  <span className="ml-2 rounded-full bg-indigo-50 px-2 py-1 font-medium text-indigo-700 text-xs">
                    Viewing as: {role?.replace('_', ' ').toUpperCase() || 'GUEST'}
                  </span>
                </p>
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="font-bold text-2xl text-slate-900">{mockData.totalProducts}</p>
                    </div>
                  ...
                  {role === 'salesperson' && (
                    <p className="mt-2 text-slate-400 text-xs">Your uploaded products</p>
                  )}
                  {role === 'factory_admin' && (
                    <p className="mt-2 text-slate-400 text-xs">Products from your factories</p>
                  )}
                </div>
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="font-bold text-2xl text-slate-900">{mockData.totalOrders}</p>
                    </div>
                    <div className="rounded-full bg-green-100 p-3">
                      <TrendingUp className="text-green-600" size={20} />
                    </div>
                  </div>
                  <p className="mt-2 text-green-600 text-xs">+12% from last month</p>
                </div>
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="font-bold text-2xl text-slate-900">{formatCurrency(mockData.totalRevenue)}</p>
                    </div>
                    <div className="rounded-full bg-yellow-100 p-3">
                      <DollarSign className="text-yellow-600" size={20} />
                    </div>
                  </div>
                  <p className="mt-2 text-green-600 text-xs">+8% from last month</p>
                </div>
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="font-bold text-2xl text-slate-900">{mockData.totalUsers}</p>
                    </div>
                  ...
                  {role === 'exporter_admin' && (
                    <p className="mt-2 text-slate-400 text-xs">All users in system</p>
                  )}
                  {role === 'factory_admin' && (
                    <p className="mt-2 text-slate-400 text-xs">Users in your factories</p>
                  )}
                </div>
              </div>
                    ...
                    {mockData.monthlyData.map((data, index) => (
                      <div key={data.month} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 rounded-full bg-slate-100">
                          ...
                          >
                            <span className="ml-2">{formatCurrency(data.sales)}</span>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              <div className="rounded-lg border border-indigo-200 bg-indigo-50 p-4">
                <h4 className="font-medium text-indigo-900">Data Access Level</h4>
                <p className="mt-1 text-indigo-700 text-sm">
                  You are currently viewing analytics with <strong>{role?.replace('_', ' ')}</strong> privileges.
                  {dataScope.products === 'factory' && " Data is limited to your factory's performance."}
                  {dataScope.products === 'own' && " Data is limited to your personal performance metrics."}
                </p>
              </div>
            </div>
          </div>
