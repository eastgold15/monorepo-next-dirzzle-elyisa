      ...
      try {
        ...
        if (sku.media?.url) {
          ...
          const [existingMedia] = await db
            .select()
            .from(mediaTable)
            .where(eq(mediaTable.url, sku.media.url))
            .limit(1);
          ...
          if (existingMedia) {
            ...
            const [newMedia] = await db
              ...
              .values({
                url: sku.media.url,
                type: sku.media.type || "image",
                alt: sku.media.alt || productDesc || productName || "",
                siteId,
              })
        ...
        const [newInquiryItem] = await db
          ...
          .values({
            ...
            skuImage: mediaId, // 使用媒体表的ID而不是URL
            skuPrice: sku.price,
            paymentMethod,
        ...
        const SalesReps = await db.query.salespersonsTable.findMany({
          with: {
            assignedCategories: {
              ...
              where: inArray(
                salespersonCategoriesTable.categoryId,
                categoryIds
              ),
            },
            user: true,
        ...
        const inquiryWithItems: InquiryWithItems = {
          customerName,
          customerCompany,
          customerEmail,
          customerPhone,
