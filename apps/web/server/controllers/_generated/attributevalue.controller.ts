...
export const attributevalueController = new Elysia({
  ...
  .get(
    "/",
    ({ query, db, siteId }) =>
      attributeValueService.findAll(query, { db, siteId }),
    { query: AttributeValueContract.ListQuery }
  )
  .post(
    "/",
    ({ body, db, siteId }) =>
      attributeValueService.create(body, { db, siteId }),
    { body: AttributeValueContract.Create }
  )
  .patch(
    "/:id",
    ({ params, body, db, siteId }) =>
      attributeValueService.update(params.id, body, { db, siteId }),
    { params: t.Object({ id: t.String() }), body: AttributeValueContract.Patch }
  )
  .delete(
    "/:id",
    ({ params, db, siteId }) =>
      attributeValueService.delete(params.id, { db, siteId }),
